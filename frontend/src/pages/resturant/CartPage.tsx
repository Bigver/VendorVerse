...
const CartPage = () => {
  ...
  const handleDetail = (special: any, more: any, detail: any) => {
    Swal.fire({
      html: `${special} <br> เพิ่มเติม <br> ${more} <br> รายละเอียดเพิ่มเติม <br> ${detail}`,
    })
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${requestMethod}/resturant/order/create`, {
      ...
      if (response.status === 201) {
        ...
        window.location.href = `/resturant/${params.nameStore}/checkout`
      }
    }
    catch (error) {
      toast.error("error")
            ...
            <div className="product" key={index}>
              <div className="card">
                ...
                <div className="text">
                  ...
                  <div className="btn">
                    <button onClick={() => handleDetail(item.nameItem2, item.nameItem1, item.detail)}>detail</button>
                  </div>
